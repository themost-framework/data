...
import {DataObjectTag} from '@themost/data';
...
describe('DataObjectTag', () => {
    ...
    function insertAudience(context: DataContext) {
    ...
    it('should try to insert json objects', async () => {
        ...
        let audience = await PeopleAudience
            .asQueryable().where((x: { name: string })=> {
        ...
        const preferredName = audience.preferredName.find((x: { recordLanguage: string }) => x.recordLanguage === 'en');
