...
const {TraceUtils, Args,
    ConfigurationBase, ConfigurationStrategy,
    PathUtils, RandomUtils,
    AbstractMethodError} = require('@themost/common');
...
function interopRequireDefault(path) {
    ...
    if (hashIndex > 0) {
        let modulePath = path.substr(0, hashIndex);
        let memberName = path.substr(hashIndex + 1);
...
class DefaultModelClassLoaderStrategy extends ModelClassLoaderStrategy {
    ...
    resolve(model) {
        ...
        let executionPath = this.getConfiguration().getExecutionPath();
        ...
        if (typeof model.classPath === 'string') {
            ...
            for (let requiredModule of requireModules) {
                try {
                    let module = require(requiredModule);
                    ...
                    let error = Object.assign(new Error('Data model class module is invalid. Expected a class to be exported as default or named member'),
                        {
                            code: 'MODULE_INVALID',
            ...
            if (model.inherits != null) {
                let inheritedModel = model.context.model(model.inherits);
                ...
                let implementedModel = model.context.model(model.implements);
