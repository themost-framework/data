...
DataAttributeResolver.prototype.testNestedAttribute = function(s) {
    ...
    var matches;
    /**
     * nested attribute aggregate function with alias e.g. f(a/b/../c) as a
     */
    matches = /^(\w+\(\w+(?:\/\w+)+\))\s+as\s+([\u0020-\u007F\u0080-\uFFFF]+)$/i.exec(s);
    if (matches) {
        return { name: matches[1], property: matches[2] };
    }
    ...
    */
    matches = /^(\w+(?:\/\w+)+)\s+as\s+([\u0020-\u007F\u0080-\uFFFF]+)$/i.exec(s);
    if (matches) {
        return { name: matches[1], property: matches[2] };
    }
    /**
     * nested attribute aggregate function with alias e.g. f(a/b/../c)
     */
    matches = /^(\w+\(\w+(?:\/\w+)+\))$/i.exec(s);
    if (matches) {
